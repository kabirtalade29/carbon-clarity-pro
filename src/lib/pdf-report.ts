...
export type GwpOdpFamilySummary = {
  family: string;
  itemCount: number;
...
export type MultiGwpOdpReportData = {
  id: string;
  reportName: string;
  companyName?: string | null;
  facility?: string | null;
  ...
  notes?: string | null;
  items: GwpOdpItem[];
  familySummaries: GwpOdpFamilySummary[];
  overall: {
    totalItems: number;
    totalQuantityKg: number;
    totalOdpEq: number;
...
export function buildGwpOdpReport(data: MultiGwpOdpReportData): jsPDF {
  const doc = new jsPDF({ unit: "pt", format: "a4" });
  let pageNum = 1;
  doc.setProperties({
    title: data.reportName || "GWP & ODP Multi-Substance Assessment Report",
  });
  ...
  const drawHeader = () => {
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    doc.text("GWP & ODP Multi-Substance Assessment Report", 48, 60);
    doc.setFontSize(9);
  ...
  const drawFooter = () => {
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
      "Generated by clisomumbai (Climate Social Mumbai). Kigali Amendment & Montreal Protocol compliant.",
      48,
      815,
    );
  ...
  doc.text("1. Overall Consolidated Assessment Summary", 48, y);
  y += 18;

  const totalTonsCO2e = data.overall.totalCo2eAR5;
  const cards = [
    {
      label: "Total Gas Mass",
      value: `${(data.overall.totalQuantityKg / 1000).toLocaleString(undefined, { maximumFractionDigits: 2 })} t`,
      color: "#1F4F3A",
    },
    {
      label: "Total ODP Eq",
      value: `${data.overall.totalOdpEq.toLocaleString(undefined, { maximumFractionDigits: 4 })} t ODP`,
      color: "#5B7A45",
    },
    {
      label: "Total CO2e (AR5)",
      value: `${totalTonsCO2e.toLocaleString(undefined, { maximumFractionDigits: 2 })} t CO2e`,
      color: "#2F6F52",
    },
  ...
  // SECTION 2: SINGLE SUBSTANCE-WISE BREAKDOWN TABLE
  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  doc.setTextColor(GREEN);
  doc.text("2. Single Substance-Wise Detailed Breakdown", 48, y);
  y += 18;

  // Table Headers
  doc.setFillColor(GREEN);
  doc.rect(48, y, 499, 22, "F");
  doc.setFontSize(8);
  doc.setTextColor("#FFFFFF");
  doc.setFont("helvetica", "bold");
  doc.text("Substance Name", 54, y + 14);
  doc.text("Gas Family", 180, y + 14);
  ...
  data.items.forEach((item, idx) => {
    ...
    if (idx % 2 === 1) {
      doc.setFillColor(BONE);
      doc.rect(48, y, 499, 20, "F");
    }

    doc.setFontSize(8);
    doc.text(item.name.substring(0, 22), 54, y + 13);
    doc.text(item.group, 180, y + 13);
    doc.text(`${item.quantity} ${item.unit}`, 270, y + 13);
    doc.text(item.odpEquivalent.toFixed(4), 345, y + 13);
    doc.text(item.gwpAR5.toString(), 420, y + 13);
    doc.text(item.co2eAR5.toLocaleString(undefined, { maximumFractionDigits: 2 }), 480, y + 13);
    y += 20;
  });
  ...
  // SECTION 3: GAS FAMILY-WISE AGGREGATED SUMMARY TABLE
  if (y > 680) {
    drawFooter();
    doc.addPage();
    pageNum++;
  ...
  doc.setFontSize(12);
  doc.setTextColor(GREEN);
  doc.text("3. Gas Family-Wise Aggregated Summary", 48, y);
  y += 18;
  ...
  doc.setTextColor("#FFFFFF");
  doc.setFont("helvetica", "bold");
  doc.text("Gas Family / Group", 54, y + 14);
  ...
  data.familySummaries.forEach((fam, idx) => {
    if (y > 740) {
      drawFooter();
      doc.addPage();
      pageNum++;
    ...
    doc.setFontSize(8);
    doc.text(fam.family, 54, y + 13);
    doc.text(`${fam.itemCount} Substances`, 200, y + 13);
    doc.text(fam.totalQuantityKg.toLocaleString(), 280, y + 13);
    doc.text(fam.totalOdpEq.toFixed(4), 375, y + 13);
    doc.text(fam.totalCo2eAR5.toLocaleString(undefined, { maximumFractionDigits: 2 }), 470, y + 13);
    y += 20;
  });
...
export async function downloadGwpOdpReport(data: MultiGwpOdpReportData) {
  await requireAuth();
  const doc = buildGwpOdpReport(data);
  triggerPdfDownload(doc, `clisomumbai-gwp-odp-${data.id.slice(0, 8)}.pdf`);
}
...
export function buildReport(data: ReportData): jsPDF {
  ...
  doc.setFontSize(8);
  doc.text(
    "Generated by clisomumbai (Climate Social Mumbai). Verified calculations aligned with GHG Protocol guidelines.",
    48,
    815,
  );
...
export async function downloadReport(data: ReportData) {
  ...
  triggerPdfDownload(doc, `clisomumbai-report-${data.id.slice(0, 8)}.pdf`);
}
...
export async function downloadConsolidatedReport(data: ConsolidatedReportData) {
  ...
  triggerPdfDownload(doc, `clisomumbai-consolidated-${data.id.slice(0, 8)}.pdf`);
}
